...
export const useHydroStore = create<HydroState>((set, get) => ({
  ...
  addWellMeasurement: (m) => set((state) => ({
    wellMeasurements: [{ ...m, id: `wm-${Date.now()}` }, ...state.wellMeasurements]
  })),

  addSoilSample: (s) => set((state) => ({
    soilSamples: [{ ...s, id: `soil-${Date.now()}` }, ...state.soilSamples]
  })),

  addSoilLabResult: (r) => set((state) => ({
    soilLabResults: [{ ...r, id: `slr-${Date.now()}` }, ...state.soilLabResults]
  })),

  addGeologicalObservation: (obs) => set((state) => ({
    geologicalObservations: [{ ...obs, id: `geo-${Date.now()}`, timestamp: new Date().toISOString() }, ...state.geologicalObservations]
  })),

  addHydroObservation: (obs) => set((state) => ({
    hydrogeologicalObservations: [{ ...obs, id: `hy-${Date.now()}`, timestamp: new Date().toISOString() }, ...state.hydrogeologicalObservations]
  })),

  addErtSurvey: (survey) => set((state) => ({
    ertSurveys: [{ ...survey, id: `ert-${Date.now()}` }, ...state.ertSurveys]
  })),

  addScientificReference: (ref) => set((state) => ({
    scientificReferences: [{ ...ref, id: `ref-${Date.now()}` }, ...state.scientificReferences]
  })),

  addAhpAnalysis: (ahp) => set((state) => ({
    ahpAnalyses: [{ ...ahp, id: `ahp-${Date.now()}`, createdAt: new Date().toISOString() }, ...state.ahpAnalyses]
  })),
  ...
  addGwpzResult: (gwpz) => set((state) => ({
    gwpzResults: [{ ...gwpz, id: `gwpz-${Date.now()}`, createdAt: new Date().toISOString() }, ...state.gwpzResults]
  })),

  addMarSite: (site) => set((state) => ({
    marSites: [{ ...site, id: `mar-${Date.now()}`, createdAt: new Date().toISOString() }, ...state.marSites]
  })),

  addResearchNote: (note) => set((state) => ({
    researchNotes: [{ ...note, id: `note-${Date.now()}` }, ...state.researchNotes]
  })),

  addGisLayer: (layer) => set((state) => ({
    gisLayers: [{ ...layer, id: `layer-${Date.now()}`, createdAt: new Date().toISOString() }, ...state.gisLayers]
  })),

  addFieldObservation: (obs) => set((state) => ({
    fieldObservations: [{ ...obs, id: `fo-${Date.now()}`, timestamp: new Date().toISOString() }, ...state.fieldObservations]
  })),
